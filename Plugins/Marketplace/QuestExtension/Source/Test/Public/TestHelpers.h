...
class FQESpec : public FAutomationSpecBase
{
public:

	FQESpec(const FString& InName, const bool bInComplexTask)
...
protected:
	...
	UQuestManagerComponent* CreateManager(UWorld* World)
	{
		if (!World)
		{
			return nullptr;
		...
		auto* Manager = NewObject<UQuestManagerComponent>(Actor);
		Manager->RegisterComponent();
		return Manager;
	}

	void DestroyManager(UQuestManagerComponent* Manager)
	{
		if (Manager)
		{
			Manager->GetOwner()->Destroy();
		}
	}
};

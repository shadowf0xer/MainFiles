...
UEMSObject* UEMSObject::Get(UObject* WorldContextObject)
{
	...
	//This will ensure that only one EMS object exists locally. 
	...
	UE_LOG(LogEasyMultiSave, Error, TEXT("Easy Multi Save could not be initialized"));
...
EActorType UEMSObject::GetActorType(AActor* Actor)
{
	...
	//Set to placed if the actor was already there on level load. Also works for sub-levels. Just skips saving ClassName, nothing more
	if (FMath::IsNearlyEqual(Actor->GetGameTimeSinceCreation(), Actor->GetWorld()->GetTimeSeconds()))
	{

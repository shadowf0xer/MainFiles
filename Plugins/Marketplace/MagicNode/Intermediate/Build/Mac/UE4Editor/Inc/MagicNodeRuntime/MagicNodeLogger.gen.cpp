// Copyright Epic Games, Inc. All Rights Reserved.
/*===========================================================================
	Generated code exported from UnrealHeaderTool.
	DO NOT modify this manually! Edit the corresponding .h files instead!
===========================================================================*/

#include "UObject/GeneratedCppIncludes.h"
#include "MagicNodeRuntime/Public/MagicNodeLogger.h"
#ifdef _MSC_VER
#pragma warning (push)
#pragma warning (disable : 4883)
#endif
PRAGMA_DISABLE_DEPRECATION_WARNINGS
void EmptyLinkFunctionForGeneratedCodeMagicNodeLogger() {}
// Cross Module References
	MAGICNODERUNTIME_API UEnum* Z_Construct_UEnum_MagicNodeRuntime_EMGCSeverity();
	UPackage* Z_Construct_UPackage__Script_MagicNodeRuntime();
// End Cross Module References
	static UEnum* EMGCSeverity_StaticEnum()
	{
		static UEnum* Singleton = nullptr;
		if (!Singleton)
		{
			Singleton = GetStaticEnum(Z_Construct_UEnum_MagicNodeRuntime_EMGCSeverity, Z_Construct_UPackage__Script_MagicNodeRuntime(), TEXT("EMGCSeverity"));
		}
		return Singleton;
	}
	template<> MAGICNODERUNTIME_API UEnum* StaticEnum<EMGCSeverity>()
	{
		return EMGCSeverity_StaticEnum();
	}
	static FCompiledInDeferEnum Z_CompiledInDeferEnum_UEnum_EMGCSeverity(EMGCSeverity_StaticEnum, TEXT("/Script/MagicNodeRuntime"), TEXT("EMGCSeverity"), false, nullptr, nullptr);
	uint32 Get_Z_Construct_UEnum_MagicNodeRuntime_EMGCSeverity_Hash() { return 4052582120U; }
	UEnum* Z_Construct_UEnum_MagicNodeRuntime_EMGCSeverity()
	{
#if WITH_HOT_RELOAD
		UPackage* Outer = Z_Construct_UPackage__Script_MagicNodeRuntime();
		static UEnum* ReturnEnum = FindExistingEnumIfHotReloadOrDynamic(Outer, TEXT("EMGCSeverity"), 0, Get_Z_Construct_UEnum_MagicNodeRuntime_EMGCSeverity_Hash(), false);
#else
		static UEnum* ReturnEnum = nullptr;
#endif // WITH_HOT_RELOAD
		if (!ReturnEnum)
		{
			static const UE4CodeGen_Private::FEnumeratorParam Enumerators[] = {
				{ "EMGCSeverity::Fatal", (int64)EMGCSeverity::Fatal },
				{ "EMGCSeverity::Error", (int64)EMGCSeverity::Error },
				{ "EMGCSeverity::Warning", (int64)EMGCSeverity::Warning },
				{ "EMGCSeverity::Info", (int64)EMGCSeverity::Info },
			};
#if WITH_METADATA
			const UE4CodeGen_Private::FMetaDataPairParam Enum_MetaDataParams[] = {
				{ "Comment", "//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////\n" },
				{ "Error.Comment", "//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////\n" },
				{ "Error.Name", "EMGCSeverity::Error" },
				{ "Fatal.Comment", "//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////\n" },
				{ "Fatal.Name", "EMGCSeverity::Fatal" },
				{ "Info.Comment", "//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////\n" },
				{ "Info.Name", "EMGCSeverity::Info" },
				{ "ModuleRelativePath", "Public/MagicNodeLogger.h" },
				{ "Warning.Comment", "//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////\n" },
				{ "Warning.Name", "EMGCSeverity::Warning" },
			};
#endif
			static const UE4CodeGen_Private::FEnumParams EnumParams = {
				(UObject*(*)())Z_Construct_UPackage__Script_MagicNodeRuntime,
				nullptr,
				"EMGCSeverity",
				"EMGCSeverity",
				Enumerators,
				UE_ARRAY_COUNT(Enumerators),
				RF_Public|RF_Transient|RF_MarkAsNative,
				UE4CodeGen_Private::EDynamicType::NotDynamic,
				(uint8)UEnum::ECppForm::EnumClass,
				METADATA_PARAMS(Enum_MetaDataParams, UE_ARRAY_COUNT(Enum_MetaDataParams))
			};
			UE4CodeGen_Private::ConstructUEnum(ReturnEnum, EnumParams);
		}
		return ReturnEnum;
	}
PRAGMA_ENABLE_DEPRECATION_WARNINGS
#ifdef _MSC_VER
#pragma warning (pop)
#endif
